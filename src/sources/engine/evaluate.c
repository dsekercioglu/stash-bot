...
enum
{
    ...
    KnightWeight = SPAIR(29, 10),
    BishopWeight = SPAIR(22, 1),
    RookWeight = SPAIR(38, 28),
    QueenWeight = SPAIR(67, 23),
    KS_Offset = SPAIR(12, 12),
...
scorepair_t evaluate_safety(evaluation_t *eval, color_t c)
{
    ...
    if (eval->attackers[c] >= 2)
    {
        ...
        return (create_scorepair(max(mg, 0) * mg / 512, max(eg, 0) / 32));
    }
    return (0);
}
