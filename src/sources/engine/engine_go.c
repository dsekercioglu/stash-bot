...
void        *engine_go(void *ptr)
{
    ...
    for (int iter_depth = 0; iter_depth < g_goparams.depth; ++iter_depth)
    {
        ...
        for (int pv_line = 0; pv_line < multi_pv; ++pv_line)
        {
            ...
            {
                score_t score = root_moves[pv_line].previous_score;
                _delta = 15 + (int)(15.0 * fmax(0, log(abs(score) / 100.0)));
                _alpha = max(-INF_SCORE, score - _delta);
                _beta = min(INF_SCORE, score + _delta);
            }
