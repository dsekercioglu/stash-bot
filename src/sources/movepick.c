...
static void score_root(movepick_t *mp, extmove_t *begin, extmove_t *end)
{
    while (begin < end)
    {
        root_move_t *cur = find_root_move(mp->worker->rootMoves,
            mp->worker->rootMoves + mp->worker->rootCount, begin->move);

        // Can trigger when "go searchmoves" is used
        if (cur == NULL)
            begin->score = -NO_SCORE;

        // Transform the node count into a score via a logarithmic scale
        else
