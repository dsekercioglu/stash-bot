...
const scorepair_t CastlingBonus = SPAIR(75, -61);
const scorepair_t Initiative = SPAIR(12, 7);

// King Safety eval terms

const scorepair_t KnightWeight    = SPAIR(  50,  75);
const scorepair_t BishopWeight    = SPAIR(  35,-312);
const scorepair_t RookWeight      = SPAIR(  32,-813);
const scorepair_t QueenWeight     = SPAIR(  62, 626);
const scorepair_t AttackWeight    = SPAIR(  19, 219);
const scorepair_t WeakKingZone    = SPAIR(   9,-458);
const scorepair_t SafeKnightCheck = SPAIR(  38,-184);
const scorepair_t SafeBishopCheck = SPAIR(  52,  35);
const scorepair_t SafeRookCheck   = SPAIR(  37,  86);
const scorepair_t SafeQueenCheck  = SPAIR(  52, -39);
const scorepair_t SafetyOffset    = SPAIR( -71,-697);

// Knight eval terms

const scorepair_t KnightShielded = SPAIR(4, 27);
const scorepair_t KnightOutpost = SPAIR(8, -20);
const scorepair_t KnightCenterOutpost = SPAIR(22, 0);
const scorepair_t KnightSolidOutpost = SPAIR(9, 31);

// Bishop eval terms

const scorepair_t BishopPairBonus = SPAIR(26, 108);
const scorepair_t BishopShielded = SPAIR(6, 22);

// Rook eval terms

const scorepair_t RookOnSemiOpenFile = SPAIR(17, 24);
const scorepair_t RookOnOpenFile = SPAIR(35, 9);
const scorepair_t RookXrayQueen = SPAIR(9, 6);

const scorepair_t MobilityN[9] = {
    SPAIR( -62,  10), SPAIR( -45, -67), SPAIR( -40,  19), SPAIR( -36,  64),
    SPAIR( -27,  72), SPAIR( -20,  85), SPAIR( -13,  90), SPAIR( -11,  90),
    SPAIR(   0,  75)
};

const scorepair_t MobilityB[14] = {
    SPAIR( -48, -25), SPAIR( -53, -80), SPAIR( -36, -41), SPAIR( -35,   2),
    SPAIR( -26,  26), SPAIR( -19,  44), SPAIR( -14,  55), SPAIR( -13,  61),
    SPAIR( -11,  60), SPAIR( -11,  66), SPAIR(   2,  65), SPAIR(   9,  51),
    SPAIR(  -7,  56), SPAIR( -21,  37)
};

const scorepair_t MobilityR[15] = {
    SPAIR( -80,   9), SPAIR( -58, -14), SPAIR( -51,  45), SPAIR( -51,  92),
    SPAIR( -45, 101), SPAIR( -42, 116), SPAIR( -35, 124), SPAIR( -32, 126),
    SPAIR( -27, 134), SPAIR( -24, 142), SPAIR( -15, 143), SPAIR( -15, 147),
    SPAIR(  -5, 139), SPAIR(  10, 132), SPAIR(  11, 119)
};

const scorepair_t MobilityQ[28] = {
    SPAIR(-105,-193), SPAIR( -15, 136), SPAIR(  15, 167), SPAIR(   4, 133),
    SPAIR(  -6, 109), SPAIR(   3,  88), SPAIR(   2, 120), SPAIR(   4, 149),
    SPAIR(   6, 168), SPAIR(  10, 176), SPAIR(  15, 191), SPAIR(  14, 206),
    SPAIR(  20, 198), SPAIR(  21, 202), SPAIR(  24, 208), SPAIR(  24, 213),
    SPAIR(  22, 210), SPAIR(  21, 221), SPAIR(  36, 205), SPAIR(  36, 209),
    SPAIR(  38, 204), SPAIR(  40, 195), SPAIR(  38, 217), SPAIR(  38, 191),
    SPAIR(  49, 180), SPAIR(  26, 207), SPAIR(  24, 205), SPAIR(  94, 196)
};
