...
void init_search_tables(void)
{
    for (int d = 1; d < 64; ++d)
        for (int m = 1; m < 64; ++m)
        {
            Reductions[0][d][m] =  0.99 /* LMR_NB */ + log(d) * log(m) / 3.00 /* LMR_NK */;
            Reductions[1][d][m] = -1.37 /* LMR_QB */ + log(d) * log(m) / 1.28 /* LMR_QK */;
        }

    for (int d = 1; d < 7; ++d)
    {
        Pruning[1][d] = +2.60 /* LMP_IB */ + 3.78 /* LMP_IK */ * pow(d, 1.05 /* LMP_IP */);
        Pruning[0][d] = -2.16 /* LMP_NB */ + 3.02 /* LMP_NK */ * pow(d, 0.84 /* LMP_NP */);
    }
}
...
__retry:
            ...
            if (bound == UPPER_BOUND)
            {
                ...
                delta += delta * 301 /* IIW_UK */ / 1024 + 0 /* IIW_UB */;
                goto __retry;
            }
    ...
    board_t *board, int depth, score_t alpha, score_t beta, searchstack_t *ss, bool pvNode)
{
    ...
    if (!pvNode && depth <= 1 /* RZ_D */ && ss->staticEval + 160 /* RZ_B */ + 27 /* RZ_K */ * depth * depth <= alpha)
    ...
    if (!pvNode && depth <= 9 /* FPP_D */ && eval - 89 /* FPP_K */ * depth + 69 /* FPP_I */ * improving >= beta && eval < VICTORY)
        return (eval);
        ...
        && eval >= beta && eval >= ss->staticEval && board->stack->material[board->sideToMove])
    {
        ...
        int R = (3504 /* NMP_B */ + 333 /* NMP_D */ * depth) / 1024 + min((eval - beta) / 125 /* NMP_E */, 5 /* NMP_M */);
    ...
    if (!rootNode && !found && depth >= 3 /* IIR_D */ - 1 /* IIR_PV */ * pvNode) --depth;
...
__main_loop:
    ...
    while ((currmove = movepick_next_move(&mp, skipQuiets)) != NO_MOVE)
    {
        ...
        if (!rootNode && bestScore > -MATE_FOUND)
        {
            ...
            if (depth <= 4 /* FPC_D */ && !inCheck && isQuiet && eval + 230 /* FPC_B */ + 81 /* FPC_K */ * depth <= alpha)
                skipQuiets = true;
...
score_t qsearch(board_t *board, score_t alpha, score_t beta, searchstack_t *ss, bool pvNode)
{
    ...
    const score_t futilityBase = bestScore + 110 /* QSF_B */;
