...
score_t search(board_t *board, int depth, score_t alpha, score_t beta, searchstack_t *ss, bool pvNode)
{
    ...
    if (!pvNode && depth == 1 && ss->staticEval + 151 <= alpha)
    ...
    if (!pvNode && depth <= 8 && eval - 90 * depth + 77 * improving >= beta && eval < VICTORY)
        return (eval);
        ...
        && board->stack->material[board->sideToMove])
    {
        ...
        int R = (801 + depth * 71) / 256 + min((eval - beta) / 124, 4);
...
__main_loop:
    ...
    while ((currmove = movepick_next_move(&mp, skipQuiets)) != NO_MOVE)
    {
        ...
        if (!rootNode && bestScore > -MATE_FOUND)
        {
            ...
            if (depth <= 4 && !inCheck && isQuiet && eval + 227 + 79 * depth <= alpha)
                skipQuiets = true;

            // SEE Pruning.

            if (depth <= 7 && !see_greater_than(board, currmove, (isQuiet ? -75 * depth : -25 * depth * depth)))
                continue ;
        }
