...
typedef int32_t capture_history_t[PIECE_NB][SQUARE_NB][PIECETYPE_NB];
...
INLINED void add_cp_history(capture_history_t hist, piece_t pc, square_t to, piecetype_t captured, int32_t bonus)
{
    int32_t *entry = &hist[pc][to][captured];

    *entry += bonus - *entry * abs(bonus) / HistoryResolution;
}

INLINED score_t get_cp_history_score(const capture_history_t hist, piece_t pc, square_t to, piecetype_t captured)
{
